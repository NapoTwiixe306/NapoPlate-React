            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-6">
              <div className="bg-gray-300 h-40 p-6 rounded flex flex-col justify-center">
                ...
                {isLoadingTokenData ? (
                  ...
                  <>
                    <p>Nom: <span className="font-bold">{tokenData?.name}</span></p>
                    <p>Symbol: <span className="font-bold">{tokenData?.symbol.toUpperCase()}</span></p>
                    <p>Décimal: <span className="font-bold">{tokenData?.decimals || "Non disponible"}</span></p>
                  </>
                )}
              </div>
              <div className="bg-gray-300 h-40 p-6 rounded flex flex-col justify-between">
                ...
                {isLoadingTokenData ? (
                  ...
                  <>
                    <p>Volume 24h: <span className="font-bold">$ {tokenData?.market_data?.total_volume?.usd?.toLocaleString()}</span></p>
                    <p>Variation 24h: <span className={`font-bold ${tokenData?.market_data?.price_change_percentage_24h && tokenData?.market_data?.price_change_percentage_24h > 0 ? 'text-green-600' : 'text-red-500'}`}>
                        {tokenData?.market_data?.price_change_percentage_24h?.toFixed(2)}%
                      </span>
                    </p>
                    <p>Capitalisation: <span className="font-bold">$ {tokenData?.market_data?.market_cap?.usd?.toLocaleString()}</span></p>
                  </>
                )}
              </div>
            </div>
